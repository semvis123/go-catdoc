...
/************************************************************************/
int to_unicode(uint16_t *charset, int c) { return charset[c]; }
/************************************************************************/
...
int get_utf8(FILE *f, long *offset, long fileend) {
  ...
  if (c < 0xE0) {
    if (catdoc_read(buf + 1, 1, 1, f) <= 0)
      return EOF;
    return (((c & 0x1F) << 6) | ((char)buf[1] & 0x3F));
  }
  if (c < 0xF0) {
    if (catdoc_read(buf + 1, 1, 2, f) <= 2)
      return (int)EOF;
...
char *convert_char(int uc) {
  ...
  char *mapped;
  int c;
  if ((mapped = map_subst(spec_chars, uc)))
    return mapped;
  if (target_charset) {
    ...
    if ((mapped = map_subst(replacements, uc)))
      return mapped;
    if (unknown_as_hex) {
...
struct cp_map cp_to_charset[] = {{10000, "mac-roman"},
                                 {10001, "mac-japanese"},
                                 {10002, "mac-tchinese"},
                                 {10003, "mac-korean"},
                                 {10004, "mac-arabic"},
...
const char *charset_from_codepage(unsigned int codepage) {
  ...
  if (codepage == 1200 || codepage == 1201) {
    ...
    for (cp = cp_to_charset; cp->codepage != 0 && cp->codepage != codepage;
         cp++)
      ;
