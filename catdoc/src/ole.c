...
FILE *ole_init(FILE *f, void *buffer, size_t bufSize) {
  ...
  while ((mblock >= 0) && (i < msat_size)) {
    ...
    if ((newbuf = realloc(tmpBuf, sectorSize * (i + 1) + MSAT_ORIG_SIZE)) !=
        NULL) {
      tmpBuf = newbuf;
    } else {
    ...
    if (fread(tmpBuf + MSAT_ORIG_SIZE + (sectorSize - 4) * i, 1, sectorSize,
              newfile) != sectorSize) {
  ...
  if (sbdStart > 0) {
    ...
    while (1) {
      ...
      if (sbdCurrent < 0 || sbdCurrent >= fileLength / sectorSize)
        break;
    }
  ...
  if (propStart >= 0) {
    ...
    while (1) {
      ...
      errno = 0;
      if (fread(properties + propLen * sectorSize, 1, sectorSize, newfile) !=
          sectorSize) {
        if (errno != 0) {
      ...
      if (propCurrent < 0 || propCurrent >= bbdSize / 4) {
        break;
      }
    }
...
int rightOleType(unsigned char *oleBuf) {
  return (oleBuf[0x42] == 1 || oleBuf[0x42] == 2 || oleBuf[0x42] == 3 ||
          oleBuf[0x42] == 5);
}
 ...
 *
 * @param name buffer for name converted to us-ascii should be at least 33 chars
 * long
...
FILE *ole_readdir(FILE *f) {
  ...
  if (e->startBlock >= 0 && e->length >= 0 &&
    ...
    while (1) {
      ...
      if (e->numOfBlocks >= chainMaxLen) {
        long int *newChain;
        chainMaxLen += 25;
        if ((newChain = realloc(e->blocks, chainMaxLen * sizeof(long int))) !=
            NULL) {
          e->blocks = newChain;
        } else {
          perror("Properties realloc error");
  ...
  if (e->length >
      (e->isBigBlock ? sectorSize : shortSectorSize) * e->numOfBlocks)
...
long int calcFileBlockOffset(oleEntry *e, long int blk) {
  ...
  if (e->isBigBlock) {
    ...
    /* 		fprintf(stderr, "calcoffset: e->name=%s e->numOfBlocks=%ld
       length=%ld sbdSecNum=%ld rootEntry->blocks=%p\n", e->name,
       e->numOfBlocks, e->length, sbdSecNum, rootEntry->blocks);*/
...
size_t ole_read(void *ptr, size_t size, size_t nmemb, FILE *stream) {
  ...
  /* 	fprintf(stderr, "llen=%ld toReadBlocks=%ld toReadBytes=%ld
   * bytesInBlock=%ld blockNumber=%ld modBlock=%ld\n", */
  /* 			llen, toReadBlocks, toReadBytes, bytesInBlock,
   * blockNumber, modBlock); */
 ...
 * pointer
 */
int my_feof(FILE *f) { return feof(f); }
#define FEOF my_feof
#else
#define FEOF feof
#endif
