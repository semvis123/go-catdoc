...
int prepare_path_buf(char *path_buf, const char *start, const char *end) {
  if (*start == '~' && start[1] == DIR_SEP) {
    ...
    start++;
    if (!home) {
      if (end - start > PATH_BUF_SIZE)
        return 0;
      strncpy(path_buf, start, end - start);
      path_buf[end - start] = 0;
    } else {
      int l = strlen(home);
      if (l + (end - start) > PATH_BUF_SIZE)
        return 0;
  ...
  if (!*path_buf) {
    ...
    strcpy(path_buf,
           add_exe_path(
               path_buf)); /* safe, becouse
                                                                          add_exe_path
                              knows about PATH_BUF_SIZE */
#endif
  }
  return 1;
}
...
char *find_file(char *name, const char *path) {
  ...
  for (p = path; p; p = (q ? (q + 1) : NULL)) {
    q = strchr(p, LIST_SEP);
    if (q) {
      if (!prepare_path_buf(path_buf, p, q))
        continue;
    } else {
      if (!prepare_path_buf(path_buf, p, p + strlen(p)))
        continue;
    }
...
char *exe_dir(void) {
  ...
  char *q;
  strcpy(pathbuf,
         _argv[0]); /* DOS ensures, that our exe path is no
...
char *add_exe_path(const char *name) {
  ...
  if (strchr(name, '%')) {
    ...
    if (strlen(name) - 1 + strlen(mypath) >= PATH_BUF_SIZE) {
      fprintf(stderr,
              "Invalid config file. file name \"%s\" too long "
              "after substitution\n",
              name);
      exit(1);
    }
...
void list_charsets(void) {
  ...
  for (p = charset_path; p; p = (q ? (q + 1) : NULL)) {
    ...
    while (!res) {
      ...
      dest = name;
      src = ffblock.ff_name;
      for (dest = name, src = ffblock.ff_name; *src && *src != '.';
           dest++, src++)
        *dest = tolower(*src);
      *dest++ = (col < 5) ? '\t' : '\n';
      if (++col > 5)
